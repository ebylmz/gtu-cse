...
public class CityPlanner implements Cloneable {
     ...
     * @param street
     */
    public CityPlanner(Street street) {
        ...
        do {
            ...
            switch (select) {
                ...
                case 2:
                    ...
                    _street.listAllStructures();
                    String[] subOptions = {"Focus", "Back"};
                    printMenu("Detailed View", subOptions, false);
                    select = getInt(">>", 0, 1);
                    if (select == 1) {
                        select = getInt("Structure:", 1, _street.getLandNumber());
                        _street.focus(_street.at(select - 1).getLoc());
                    }
                    break;
                case 3:
    ...
    private void removeMenu() {
        ...
        _street.listAllStructures();
        int select = getInt(">> ", 0, _street.getLandNumber());
        if (select != 0) {
            if (_street.remove(select - 1)) 
                System.out.printf("\nRemoved succesfully. ");
            else
               System.out.printf("\nCannot remove. ");
            enterToContinue();
