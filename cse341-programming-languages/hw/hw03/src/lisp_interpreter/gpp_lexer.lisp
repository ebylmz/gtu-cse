...
(defun lexer-from-cl ()
    ...
    (loop :for curr-char := (read-char nil) :do
        (tokenize curr-char)
        (cond 
            (exit-flag (return-from lexer-from-cl nil))
                ...
                (progn 
                    ...
                    (cond                        
                        ...
                        ((is-valuei unmached-token)
                            (push (list (parse-integer unmached-token) "VALUEI") token-list))
...
(defun parse-rational (valuef)
    (let ((i (position #\f valuef)) (num nil) (denom nil))
        (cond
            ((null i) 
                nil)
            (t 
