...
struct pid_list child_pids; //! remove

struct safe_dir *sdir;


char err_msg[ERR_MSG_LEN]; //! remove, find better solution 

pthread_t *thread_pool;

struct client_list serving_clients;
...
int main(int argc, char *argv[])
{
    ...
    int stat, max_clients, pool_size, client_id;
    ...
    clist_init(&serving_clients, max_clients);
    ...
    thread_pool = create_thread_pool(pool_size, thread_function, (void *) NULL);
    if (thread_pool == NULL) {
    ...
    while (1) {
        ...
        if (num_read != -1) {
            if (num_read < (long) sizeof(struct request_header)) {
                ...
                switch (req_head.type) {
                    case REQ_CONNECT:
                        /* Check the availablty of the system */
                        break;
                    case REQ_DISCONNECT:
                        break;
                    case REQ_GET:
...
race_condtion(int log_fd, pid_t client_pid, const char *info)
{
    char log[LOG_LEN];
