...
int InterruptManager::NumTimerInterrupt = 0;
SystemBuffer InterruptManager::stdin;
bool InterruptManager::IOLock = false;
...
void SystemBuffer::write(char *str) 
{
    while ((buff[writeEnd++] = *str) != '\0') {
        ++str;
        // buffer overflow, come back to start point 
        if (writeEnd == BUFF_SIZE)
            writeEnd = 0;
    } 
}

void SystemBuffer::write(char c) 
{
    //TODO: consider the buffer overflow sitituation (writeEnd < readEnd)
    buff[writeEnd++] = c;
}

char *SystemBuffer::read(char delim) 
{  
    //TODO: consider the buffer overflow sitituation (writeEnd < readEnd)
    if (readEnd == writeEnd || buff[writeEnd - 1] != delim) // nothing to read
        return nullptr;

    // to be in safe place put null deliminator (readEnd == writeEnd)
    buff[writeEnd] = '\0'; 
    char *str = buff + readEnd;
    //! adjust the readend
    readEnd = writeEnd;
    return str;
}

char *SystemBuffer::read() 
{
    //TODO: consider the buffer overflow sitituation (writeEnd < readEnd)
    if (readEnd == writeEnd) // nothing to read
        return nullptr;

    char *str = buff + readEnd;

    // move the readEnd (all strings are terminated with a null charachter)
...
void InterruptManager::setIOLock(bool lock) 
{
    IOLock = lock;
}
